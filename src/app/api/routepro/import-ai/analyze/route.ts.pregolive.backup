...
import { buildAiImportSummary, getAiImportOptimizationBlockReason } from "@/modules/routepro/server/routepro.ai-import-guards";
...
export async function POST(request: Request) {
  ...
  await extractRouteProStopsWithOpenAiVisionBatches(files);
...
const batchSummaries = batchResults.map((batch) => ({
  ...
  extractedStops: batch.stops.length,
    ...
    const recoveryPlan = buildRouteProAiRecoveryPlan(stops);
    const blockingReason = getAiImportOptimizationBlockReason(summary);
    ...
    routeProAiImportPreviewStore.set(preview.importId, preview);
